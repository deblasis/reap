...
2. **No directory carrying any BLOCKED-class fact may be deleted** - not
   ...
   one named carve-out (orphaned worktrees) behind a TTY-only hardened
   confirm with a capped plain-copy snapshot first. Holds and protect globs
...
What it looks like (Windows; identical on macOS and Linux - on a machine
without `gh`, the clean repo honestly verdicts `gh-unavailable` instead of
`clean + fully pushed`, and nothing else changes):

```text
=== scan - what exists, and what each dir is

> reap scan
reap scan                                                          2026-09-10 17:27
roots: C:/Users/.../reap-demo-SVTP5F/world  4 dirs, 0.0 GB logical

BLOCKED 1 dirs    0.0 GB   waiting on you: push or discard
    0.0 GB  ...world\dirty-repo 1 tracked-modified, 0 untracked
         hint: commit+push the work, or reap discard C:\...\dirty-repo
...
> reap plan
reap will permanently delete 0 directories, logical 0.0 GB ...
dry-run: nothing will be deleted
# (the held dir is KEEP, so the plan is empty; the demo then releases the
#  hold with `reap unhold` and the plan picks the scratch dir back up)

=== apply - delete the SAFE set, with the write-ahead audit trail

> reap apply --yes
deleted 1 dirs (logical 0.0 GB, expected reclaim ~0.0 GB), excluded 0 ..., skipped 0 ...: none

=== discard - resolve a BLOCKED dir: quarantine it, then delete

> reap discard C:\...\dirty-repo --yes
...
> reap quarantine restore 20260910-172551-dirty-repo --to ...\recovered
restored ... (capture ref refs/reap/capture-20260910-172551 materialized ...)
recovered file says: uncommitted edits - the only copy

=== log - every decision reap made, write-ahead, one line per event

> reap log
2026-09-10T14:25:50  intent    begin     C:\...\world\scratch-old
2026-09-10T14:25:50  result    deleted   C:\...\world\scratch-old
2026-09-10T14:25:50  envelope  run end: deleted=1 skipped=0
```
...
`%LOCALAPPDATA%\reap\state\config.json` (or `$REAP_DIR/config.json`):
...
AGENT-RULE.md documents the pinned agent-facing contract (the carve-out is
TTY-only, `--yes` never unlocks it).
