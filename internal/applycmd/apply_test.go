...
func TestReverifyTripwireSkipsFreshDir(t *testing.T) {
	...
	cfg := config.Default()
	rv := Reverify(target, "scratch-idle", false, cfg, Deleter{}, config.ExpandRoots(cfg.Protect), nil, nil, nil, nil)
...
func TestReverifyProtectedSkips(t *testing.T) {
	...
	rv := Reverify(target, "scratch-idle", false, cfg, Deleter{}, protected, nil, nil, nil, nil)
